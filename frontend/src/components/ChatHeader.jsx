        ...
        {loading ? (
          ...
          <div className="p-4 flex flex-col gap-5">
            ...
            <InfoSection title="Description">
              {info?.description
                ? <p className="text-[12px] text-slate-500 leading-relaxed m-0">{info.description}</p>
...
function InfoRow({ icon, label, value, accent }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className={`text-[12px] font-medium ${accent ? "text-amber-500" : "text-slate-700"}`}>
        {String(value)}
      </span>
    </div>
  );
      ...
      <div className="h-[50px] flex items-center px-4 border-b border-slate-200 flex-shrink-0 bg-white justify-between relative z-10">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-[14px] font-semibold text-slate-800">
            <i className={`ti ${isPrivate ? "ti-lock" : "ti-hash"} text-[14px] text-slate-400`} />
